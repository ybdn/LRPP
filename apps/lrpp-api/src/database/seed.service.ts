...
export class DatabaseSeedService implements OnModuleInit {
  ...
  async seedDatabase(options: { skipIfNotEmpty?: boolean } = {}) {
    ...
    try {
      ...
      this.logger.error('Failed to seed database', error instanceof Error ? error.stack : String(error));
      throw error;
    }
  }
  ...
  private async seedPvs() {
    ...
    const files = (await fs.readdir(pvsDir)).filter((file) => file.endsWith('.json'));
  ...
  private async seedSectionsAndBlocks(pvData: PvSeed) {
    ...
    if (pvData.hasNotification) {
      sections.push({ label: 'notification', title: 'Notification des droits', order: 3 });
    }

    if (pvData.hasDeroulement) {
      sections.push({ label: 'deroulement', title: 'Déroulement de la mesure', order: 4 });
    }

    sections.push({ label: 'elements_fond', title: 'Éléments de fond', order: sections.length + 1 });
    ...
    await this.createSimpleBlock(pvData, sectionRefs['motivation'], pvData.motivation, ['motivation']);
    await this.createSimpleBlock(pvData, sectionRefs['notification'], pvData.notification, ['notification', 'droits']);
    await this.createSimpleBlock(pvData, sectionRefs['deroulement'], pvData.deroulement, ['deroulement', 'mesure']);
    await this.createSimpleBlock(pvData, sectionRefs['elements_fond'], pvData.elementsFond, ['elements_fond']);
  }
