...
export const MODE_COMPLETION_PROFILES: Record<
  TrainingMode,
  SectionCompletionProfile[]
> = {
...
export function resolveGapDensity(
  level: number | undefined,
  profile?: SectionCompletionProfile,
) {
  if (profile?.gapDensity !== undefined) {
    return profile.gapDensity;
  }
  const base =
    DEFAULT_GAP_DENSITY_BY_LEVEL[level ?? 1] ?? DEFAULT_GAP_DENSITY_BY_LEVEL[1];
  if (
    profile?.minGapDensity !== undefined &&
    profile?.maxGapDensity !== undefined
  ) {
